...
backward_val = np.zeros(result.shape)
...
if padding > 0:
    padded_image = np.zeros((no_of_channels, result.shape[1] + (2 * padding), result.shape[2]+(2 * padding)))
...
print("padded image\n", padded_image)
backward_val = np.zeros(padded_image.shape)

activated_values = np.zeros((padded_image.shape[0], padded_image.shape[1],padded_image.shape[2]))
new_width = int(
...
for channel in range(0, no_of_channels):
    ...
    for i in range(0, new_width):
        ...
        while d <= padded_image.shape[2]:
            ...
            max_val = np.max(padded_image[channel][a:b, c:d])
...
#backward for the pooling_max
print(backward_val.shape)
for channel in range(0, no_of_channels):
    a = 0
    b = kernal_size[0]
    for i in range(0, new_width):
        ...
        while d <= padded_image.shape[2]:
            ...
            # print("kernal", new_kernal[channel])
            max_val = np.max(padded_image[channel][a:b, c:d])
            nik = np.argwhere(padded_image[channel][a:b, c:d] == max_val)
            # new_back_prop = np.zeros(result.shape)
            
            print(max_val)
            print(nik[0][0]+a)
            print(nik[0][1]+c)
            dimension_0 = nik[0][0]+a
            dimension_1 = nik[0][1]+c
            # print(dimension_0)
            # print(dimension_1)
            backward_val[channel][dimension_0][dimension_1]=max_val
